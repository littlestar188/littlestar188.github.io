$(function() {
	...
	var nav = $('.head-bottom').find('.nav');
	var navItem = $('.nav').find('.nav-item');
	var navList = $('.nav').find('.nav-list');
	navItem.hover(function() {
		var index = $(this).index();
		console.log(index);
		navList.eq(index).css({
			top: 53 * index - 1
		})
		navList.eq(index).show();
		navList.eq(index).parents().siblings().children('.nav-list').siblings().hide();
		//console.log(navList);
	}, function() {
		var index = $(this).index();
		navList.eq(index).hide();
